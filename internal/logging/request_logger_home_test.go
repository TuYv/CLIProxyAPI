...
func TestFileRequestLogger_HomeEnabled_ForwardsWhenRequestLogEnabled(t *testing.T) {
	...
	if got.Headers == nil || got.Headers["Authorization"][0] != "Bearer se...et" {
		t.Fatalf("headers.authorization = %+v, want Bearer se...et", got.Headers["Authorization"])
	}
	if got.Headers == nil || got.Headers["Cookie"][0] != "sess...cret" {
		t.Fatalf("headers.cookie = %+v, want sess...cret", got.Headers["Cookie"])
	}
