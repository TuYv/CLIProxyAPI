...
func TestDecodeCapturedZstdRequestBodyCapsDecodedSize(t *testing.T) {
	payload := bytes.Repeat([]byte("a"), int(maxDecodedCapturedRequestBodyBytes)+10)
	var compressed bytes.Buffer
	encoder, errNewWriter := zstd.NewWriter(&compressed)
	if errNewWriter != nil {
		t.Fatalf("zstd.NewWriter: %v", errNewWriter)
	...
	decoded, errDecode := decodeCapturedZstdRequestBody(compressed.Bytes())
	if errDecode != nil {
		t.Fatalf("decodeCapturedZstdRequestBody: %v", errDecode)
	}
	if int64(len(decoded)) != maxDecodedCapturedRequestBodyBytes {
		t.Fatalf("decoded length = %d, want %d", len(decoded), maxDecodedCapturedRequestBodyBytes)
	}
}
