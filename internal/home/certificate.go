...
func ConfigFromJWT(ctx context.Context, rawJWT string) (config.HomeConfig, error) {
	...
	paths, errPaths := defaultCertificatePaths(claims)
	if errPaths != nil {
...
func defaultCertificatePaths(claims homeJWTClaims) (certificatePaths, error) {
	homeDir, errHome := os.UserHomeDir()
	if errHome != nil {
		return certificatePaths{}, errHome
	}
	dir := filepath.Join(homeDir, ".cli-proxy-api")
	clusterPart := certificatePathPart(claims.ClusterID)
	certificatePart := certificatePathPart(claims.CertificateID)
	caPart := certificatePathPart(normalizeFingerprint(claims.CAFingerprint))
	return certificatePaths{
		Dir:        dir,
		ClientCert: filepath.Join(dir, fmt.Sprintf("client-crt-%s-%s-%s.pem", clusterPart, certificatePart, caPart)),
		ClientKey:  filepath.Join(dir, fmt.Sprintf("client-key-%s-%s-%s.pem", clusterPart, certificatePart, caPart)),
		CACert:     filepath.Join(dir, fmt.Sprintf("home-ca-crt-%s-%s.pem", clusterPart, caPart)),
	}, nil
}

func certificatePathPart(value string) string {
